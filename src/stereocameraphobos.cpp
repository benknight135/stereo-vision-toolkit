...
bool StereoCameraPhobos::find_systems(void){
    ...
    if (nCameras < 2) {
        qDebug() << "Fewer than two cameras connected";
        return false;
    }
...
QList<qint64> StereoCameraPhobos::load_serials(QString filename) {
  QList<qint64> serials;
...
bool StereoCameraPhobos::setCameras(CameraImagingSource *left_camera, CameraImagingSource *right_camera){
...
void StereoCameraPhobos::setup_cameras(){
...
void StereoCameraPhobos::loadRightSettings(){
    right_camera->showProperties();
}

bool StereoCameraPhobos::capture(){
...
StereoCameraPhobos::~StereoCameraPhobos(){
   return;
}
